...
public:
	...
	bool save_to_txt(fstream& f_out)const;
...
void ListTwoTemplate<Type>::save_to_bin_file(FILE* file)const
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		if constexpr (is_same<Type, MyString>::value )
		{
...
template<class Type>
bool ListTwoTemplate<Type>::save_to_txt(fstream& f_out)const
{
	if (!f_out.is_open()) return false;

	for (int i = 0; i < this->count; i++)
	{
		(*this)[i].save_to_txt(f_out);
	}
	return true;
}
...
ofstream& operator<<(ofstream& f_out, const ListTwoTemplate<Type>& obj)
{
	...
	if constexpr (is_same<Type, MyString>::value || is_same<Type, User>::value)
	{
