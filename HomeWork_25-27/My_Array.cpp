...
MyArray::MyArray(int other_size, int* other_arr) : size{ static_cast<unsigned int>(other_size) }, arr{ nullptr }, to_str_type{ nullptr }
{
	if (other_size <= 0 || other_arr == nullptr)
	{
		throw invalid_argument("Неверный размер или указатель на массив");
	}
	arr = new int[other_size];
...
std::ofstream& operator<<(std::ofstream& f_out,const MyArray& obj)
{
	if (!f_out.is_open()) return f_out;
	f_out.write((char*)&obj.size, sizeof(obj.size));
	f_out.write((char*)obj.arr, obj.size * sizeof(int));

	return f_out;
}

std::ifstream& operator>>(std::ifstream& f_in, MyArray& obj)
{
	if (!f_in.is_open()) return f_in;
	f_in.read((char*)&obj.size, sizeof(obj.size));
	delete[]obj.arr;
	obj.arr = new int[obj.size];
	f_in.read((char*)obj.arr, obj.size * sizeof(int));
	return f_in;
}
...
void MyArray::save_to_txt(std::fstream& f_out)const
{
...
void MyArray::save_to_bin_file(FILE* file) const
{
	//запись длины массива
	fwrite(&this->size, sizeof(this->size), 1, file);


	//запись содержимого массива
	fwrite(this->arr, sizeof(this->size), this->size, file);


}

void MyArray::read_from_bin_file(FILE* file)
{
	if (this->arr != nullptr)
	{
		delete[]this->arr;
...
//Преобразование к char* работает, все хорошо.
//Как вариант, короче с использованием функций :
//
//if (!this->size) {
//	return 0;
//}
...
//	}
//}
//to_str_type = new char[strlen(buffer) + 1];
//strcpy_s(to_str_type, strlen(buffer) + 1, buffer);
//return to_str_type;
